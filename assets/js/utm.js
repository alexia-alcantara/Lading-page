/**
 * Adiciona utm tags e outros parametros nos links
 * @param  {[type]} typeof URI           [description]
...
$(document).ready(function() {
  var urlQueryParams = URI.parseQuery(window.location.search);

  $("a").each(function() {
    if ($(this).attr("href")) {
      var anchorUrl = URI($(this).attr("href"));
      var anchorQueryParams = URI.parseQuery(anchorUrl.search());
      var finalParams = $.extend(urlQueryParams, anchorQueryParams);

      $.each(anchorQueryParams, function(field, fieldIndex) {
        anchorUrl.removeSearch(fieldIndex);
      });

      anchorUrl.addSearch(finalParams);
